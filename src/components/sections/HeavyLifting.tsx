  ...
  return (
    <motion.div style={{ opacity }} className="absolute inset-0 flex h-full w-full items-center px-8 md:px-20">
      <motion.div style={{ x }} className="max-w-xl">
        <span className="glass mb-4 inline-flex items-center gap-2 rounded-full border border-blue/40 px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.3em] text-blue-soft">
          Heavy Lifting Zone
        </span>
        ...
        <ul className="mt-8 grid grid-cols-2 gap-3">
          {FEATURES.map((f) => (
            <li
              key={f}
              className="glass flex items-center gap-2 rounded-lg border border-blue/25 px-3 py-2 text-sm text-bone/80"
            >
